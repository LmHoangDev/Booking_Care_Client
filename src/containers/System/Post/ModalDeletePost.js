...
import { deletePostService } from "../../../services/userService";
import * as actions from "../../../store/actions";
const mdParser = new MarkdownIt(/* Markdown-it options */);

class ModalDeletePost extends Component {
  constructor(props) {
    super(props);
    this.state = {
      id: "",
  ...
  handleSubmit = async () => {
    try {
      let res = await deletePostService({ id: this.state.id });
      console.log("res", res);
      if (res && res.message.errCode === 0) {
        toast.success("Xoá bài viết thành công!!");
        await this.props.fetchAllPostsStart();
        this.toggle();
      } else {
...
export default connect(mapStateToProps, mapDispatchToProps)(ModalDeletePost);
